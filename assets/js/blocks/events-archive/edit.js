...
export default function Edit({ attributes, setAttributes }) {
  const { eyebrow, title, lead, perPage, inquiryTitle, inquiryText, inquiryCtaText, inquiryCtaUrl } = attributes;

  return (
    <>
      <InspectorControls>
        <PanelBody title="Listing" initialOpen>
          <RangeControl label="Events per page" value={perPage} min={1} max={24} onChange={(v) => setAttributes({ perPage: v })} />
          <p style={{ fontSize: 12, color: '#777' }}>
            Every published Event renders on the front-end as a card grid, built from the Event CPT and its ACF fields.
          </p>
        </PanelBody>
        <PanelBody title="Inquiry strip" initialOpen={false}>
      ...
      <section {...useBlockProps({ className: 'events-archive-editor' })} style={{ padding: '24px 0' }}>
        ...
        <RichText tagName="h1" className="display" style={{ fontSize: 'clamp(40px, 6vw, 80px)', margin: '16px 0 0', lineHeight: 1.05 }} value={title} onChange={(v) => setAttributes({ title: v })} placeholder="Archive title…" allowedFormats={['core/italic']} />
        <RichText tagName="p" style={{ marginTop: 20, maxWidth: 540, color: 'var(--ink-2, #3d433d)', lineHeight: 1.7 }} value={lead} onChange={(v) => setAttributes({ lead: v })} placeholder="Lead paragraph…" allowedFormats={['core/bold', 'core/italic']} />

        <div style={{ margin: '28px 0', padding: '40px 0', border: '1px dashed #cdd3cd', borderRadius: 4, textAlign: 'center', color: '#7b817b', fontSize: 13 }}>
          Events card grid renders here on the front-end
        </div>
